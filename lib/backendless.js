(function () {
    ...
    Backendless._ajax_for_browser = function (config) {
        var cashingAllowedArr = ['cacheOnly', 'remoteDataOnly', 'fromCacheOrRemote', 'fromRemoteOrCache', 'fromCacheAndRemote'],
            ...
            sendRequest = function (config) {
                ...
                } else if (Backendless.Cache.exists("user-token")) {
                    xhr.setRequestHeader("user-token", Backendless.Cache.get("user-token"));
                }
                if (UIState !== null) {
    ...
    var getClassName = function () {
        var instStringified = (Utils.isFunction(this.constructor) ? this.toString() : this.constructor.toString()),
    ...
    if (Backendless.Cache.enabled) {
        Backendless.Cache.flushExpired();
    }
    ...
    DataStore.prototype = {
        _extractQueryOptions: function (otpions) {
            ...
            if (otpions.relationsDepth) {
                if (Utils.isNumber(otpions.relationsDepth)) {
                    params.push('relationsDepth=' + encodeURIComponent(Math.floor(otpions.relationsDepth)));
                }
            }
            if (otpions.relations) {
                if (Utils.isArray(otpions.relations)) {
                    params.push('loadRelations=' + (otpions.relations.length ? encodeArrayToUriComponent(otpions.relations) : "*"));
                }
            }
            return params.join('&');
        },
    ...
    UserService.prototype = {
        ...
        login: function (username, password, stayLoggedIn, async) {
            ...
            Backendless.Cache.set("stayLoggedIn", false);
            if (Utils.isBoolean(stayLoggedIn)) {
                Backendless.Cache.set("stayLoggedIn", stayLoggedIn);
            }
            var responder = extractResponder(arguments);
            var isAsync = responder != null;
        ...
        _getUserFromResponse: function (user) {
            ...
            for (var i in user) {
                if (user.hasOwnProperty(i)) {
                    if (i == 'user-token') {
                        if (Backendless.Cache.get("stayLoggedIn")) {
                            Backendless.Cache.set("user-token", user[i]);
                        }
                        continue;
                    }
                    newUser[i] = user[i];
        ...
        logout: function (async) {
            ...
            Backendless.Cache.remove("user-token");
            if (isAsync) {
                return result;
            }
            else {
